            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-black text-slate-950">
                    Onde seu dinheiro saiu neste mês
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-slate-500">
                    Essa área não decide se pode comprar. Ela mostra onde cortar primeiro quando o mês apertar.
                  </p>
                </div>
              ...
              {categoryChartData.length === 0 ? (
                <p className="mt-4 rounded-2xl border border-dashed border-slate-200 px-4 py-5 text-sm text-slate-500">
                  Ainda não há gastos por categoria neste mês.
                </p>
              ) : (
                <div className="mt-5 space-y-3">
                  {categoryChartData.slice(0, 6).map((item) => {
                    const percentage = Math.min(Math.max(Number(item.percentage || 0), 0), 100);
                    const isMainLeak = item.percentage >= 35;
                    const isAttention = item.percentage >= 20 && item.percentage < 35;
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs leading-5 text-slate-500">
                              {isMainLeak
                                ? "Principal ponto de corte do mês."
                                : isAttention
                                ? "Categoria pedindo atenção."
